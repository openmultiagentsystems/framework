...
@p__297[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art([],_1860).
...
@p__299[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art(art_env(W,"",_1861),Try) <- .concat("/main/",W,FullW); joinWorkspace(FullW,_1862); .print("joinned workspace ",FullW).
...
@pfunction2[atomic,source(self),url("file:src/agt/default_agent.asl")] +main <- .print("Hello there. I'm a regular agent"); ?agent_id(AGENTID); ?path(PATH); .random(R); +my_testing(R); ?my_testing(R); .print("R: ",R); .abolish(agent_id(_1866)); .abolish(path(_1867)); ?sugar(SUGAR); ?metabolism(METABOLISM); ?vision(VISION); .print("Sugar ",SUGAR); .print("Metabolism ",METABOLISM); .print("Vision ",VISION); .abolish(sugar(_1868)); .abolish(metabolism(_1869)); .abolish(vision(_1870)); .my_name(MYNAME); .concat("src/agt/list/",MYNAME,".asl",NAME); .save_agent(NAME,[start,say(hello)]); .print("Saved my information on file. Sending message to remove agent from simulation"); .send(killer_agent,tell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)); .send(killer_agent,untell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)).
@p__302[source(self),url("file:src/agt/default_agent.asl")] +free : (gsize(_1871,W,H) & (jia.random(RX,(W-1)) & jia.random(RY,(H-1)))) <- .print("I am going to go near (",RX,",",RY,")"); !go_near(RX,RY).
...
@p__305[source(self),url("file:src/agt/default_agent.asl")] +!go_near(X,Y) : free <- -near(_1872,_1873); -last_dir(_1874); !near(X,Y).
...
@p__311[source(self),url("file:src/agt/default_agent.asl")] +!next_step(X,Y) : not (pos(_1875,_1876)) <- !next_step(X,Y).
...
@pih1[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : .desire(near(_1877,_1878)) <- .print("Dropping near(_,_) desires and intentions to handle ",Gold); .drop_desire(near(_1879,_1880)); !init_handle(Gold).
@pih2[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : pos(X,Y) <- .print("Going for ",Gold); !!handle(Gold).
@pfunction[atomic,source(self),url("file:src/agt/default_agent.asl")] +!handle(gold(X,Y)) : not (free) <- .print("Handling ",gold(X,Y)," now."); !pos(X,Y); !ensure(pick,gold(X,Y)); ?depot(_1881,DX,DY); !pos(DX,DY); !ensure(drop,0); .print("Finish handling ",gold(X,Y)); ?score(S); -+score((S+1)); .send(leader,tell,dropped); +main.
...
@p__318[source(self),url("file:src/agt/default_agent.asl")] +!ensure(pick,_1882) : (pos(X,Y) & gold(X,Y)) <- pick; ?carrying_gold; -gold(X,Y).
@p__319[source(self),url("file:src/agt/default_agent.asl")] +!ensure(drop,_1883) : (carrying_gold & (pos(X,Y) & depot(_1884,X,Y))) <- drop.
...
@p__322[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : not (gold(_1885,_1886)) <- -+free.
@p__323[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : gold(_1887,_1888) <- .findall(gold(X,Y),gold(X,Y),LG); !calc_gold_distance(LG,LD); .length(LD,LLD); (LLD > 0); .print("Gold distances: ",LD,LLD); .min(LD,d(_1889,NewG)); .print("Next gold is ",NewG); !!handle(NewG).
...
@p__327[source(self),url("file:src/agt/default_agent.asl")] +!calc_gold_distance([_1890|R],RD) <- !calc_gold_distance(R,RD).
@p__328[source(self),url("file:src/agt/default_agent.asl")] +end_of_simulation(S,_1891) <- .drop_all_desires; .abolish(gold(_1892,_1893)); .abolish(picked(_1894)); -+free; .print("-- END ",S," --").
