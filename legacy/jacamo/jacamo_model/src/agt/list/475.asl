// beliefs and rules
kqml::bel_no_source_self(NS::Content,Ans) :- (NS::Content[|LA] & (kqml::clear_source_self(LA,NLA) & ((Content =.. [F,T,_633]) & (Ans =.. [NS,F,T,NLA])))).
...
focused(WksName,ArtName[artifact_type(Type)],ArtId) :- focusing(ArtId,ArtName,Type,_591,WksName,_592).
joinedWsp(cobj_2,mining,"/main/mining")[artifact_id(cobj_1),artifact_name(session_475),percept_type(obs_prop),source(percept),workspace("/main",cobj_0)].
joinedWsp(cobj_0,main,"/main")[artifact_id(cobj_1),artifact_name(session_475),percept_type(obs_prop),source(percept),workspace("/main",cobj_0)].
joined(WksName,WksId) :- joinedWsp(WksId,WksName,_593).
last_dir(down).
focusing(cobj_3,m2view,"mining.MiningPlanet",cobj_2,mining,"/main/mining")[artifact_id(cobj_4),artifact_name(body_475),percept_type(obs_prop),source(percept),workspace("/main/mining",cobj_2)].
...
@p__105[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art([],_588).
...
@p__107[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art(art_env(W,"",_589),Try) <- .concat("/main/",W,FullW); joinWorkspace(FullW,_590); .print("joinned workspace ",FullW).
...
@pfunction2[atomic,source(self),url("file:src/agt/default_agent.asl")] +main <- .print("Hello there. I'm a regular agent"); ?agent_id(AGENTID); ?path(PATH); .random(R); +my_testing(R); ?my_testing(R); .print("R: ",R); .abolish(agent_id(_594)); .abolish(path(_595)); ?sugar(SUGAR); ?metabolism(METABOLISM); ?vision(VISION); .print("Sugar ",SUGAR); .print("Metabolism ",METABOLISM); .print("Vision ",VISION); .abolish(sugar(_596)); .abolish(metabolism(_597)); .abolish(vision(_598)); .my_name(MYNAME); .concat("src/agt/list/",MYNAME,".asl",NAME); .save_agent(NAME,[start,say(hello)]); .print("Saved my information on file. Sending message to remove agent from simulation"); .send(killer_agent,tell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)); .send(killer_agent,untell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)).
@p__110[source(self),url("file:src/agt/default_agent.asl")] +free : (gsize(_599,W,H) & (jia.random(RX,(W-1)) & jia.random(RY,(H-1)))) <- .print("I am going to go near (",RX,",",RY,")"); !go_near(RX,RY).
...
@p__113[source(self),url("file:src/agt/default_agent.asl")] +!go_near(X,Y) : free <- -near(_600,_601); -last_dir(_602); !near(X,Y).
...
@p__119[source(self),url("file:src/agt/default_agent.asl")] +!next_step(X,Y) : not (pos(_603,_604)) <- !next_step(X,Y).
...
@pih1[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : .desire(near(_605,_606)) <- .print("Dropping near(_,_) desires and intentions to handle ",Gold); .drop_desire(near(_607,_608)); !init_handle(Gold).
@pih2[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : pos(X,Y) <- .print("Going for ",Gold); !!handle(Gold).
@pfunction[atomic,source(self),url("file:src/agt/default_agent.asl")] +!handle(gold(X,Y)) : not (free) <- .print("Handling ",gold(X,Y)," now."); !pos(X,Y); !ensure(pick,gold(X,Y)); ?depot(_609,DX,DY); !pos(DX,DY); !ensure(drop,0); .print("Finish handling ",gold(X,Y)); ?score(S); -+score((S+1)); .send(leader,tell,dropped); +main.
...
@p__126[source(self),url("file:src/agt/default_agent.asl")] +!ensure(pick,_610) : (pos(X,Y) & gold(X,Y)) <- pick; ?carrying_gold; -gold(X,Y).
@p__127[source(self),url("file:src/agt/default_agent.asl")] +!ensure(drop,_611) : (carrying_gold & (pos(X,Y) & depot(_612,X,Y))) <- drop.
...
@p__130[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : not (gold(_613,_614)) <- -+free.
@p__131[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : gold(_615,_616) <- .findall(gold(X,Y),gold(X,Y),LG); !calc_gold_distance(LG,LD); .length(LD,LLD); (LLD > 0); .print("Gold distances: ",LD,LLD); .min(LD,d(_617,NewG)); .print("Next gold is ",NewG); !!handle(NewG).
...
@p__135[source(self),url("file:src/agt/default_agent.asl")] +!calc_gold_distance([_618|R],RD) <- !calc_gold_distance(R,RD).
@p__136[source(self),url("file:src/agt/default_agent.asl")] +end_of_simulation(S,_619) <- .drop_all_desires; .abolish(gold(_620,_621)); .abolish(picked(_622)); -+free; .print("-- END ",S," --").
