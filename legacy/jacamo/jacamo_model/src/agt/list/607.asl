// beliefs and rules
kqml::bel_no_source_self(NS::Content,Ans) :- (NS::Content[|LA] & (kqml::clear_source_self(LA,NLA) & ((Content =.. [F,T,_1585]) & (Ans =.. [NS,F,T,NLA])))).
...
focused(WksName,ArtName[artifact_type(Type)],ArtId) :- focusing(ArtId,ArtName,Type,_1543,WksName,_1544).
joinedWsp(cobj_2,mining,"/main/mining")[artifact_id(cobj_1),artifact_name(session_607),percept_type(obs_prop),source(percept),workspace("/main",cobj_0)].
joinedWsp(cobj_0,main,"/main")[artifact_id(cobj_1),artifact_name(session_607),percept_type(obs_prop),source(percept),workspace("/main",cobj_0)].
joined(WksName,WksId) :- joinedWsp(WksId,WksName,_1545).
last_dir(down).
focusing(cobj_3,m2view,"mining.MiningPlanet",cobj_2,mining,"/main/mining")[artifact_id(cobj_4),artifact_name(body_607),percept_type(obs_prop),source(percept),workspace("/main/mining",cobj_2)].
gold(20,21).
...
@p__265[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art([],_1540).
...
@p__267[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art(art_env(W,"",_1541),Try) <- .concat("/main/",W,FullW); joinWorkspace(FullW,_1542); .print("joinned workspace ",FullW).
...
@pfunction2[atomic,source(self),url("file:src/agt/default_agent.asl")] +main <- .print("Hello there. I'm a regular agent"); ?agent_id(AGENTID); ?path(PATH); .random(R); +my_testing(R); ?my_testing(R); .print("R: ",R); .abolish(agent_id(_1546)); .abolish(path(_1547)); ?sugar(SUGAR); ?metabolism(METABOLISM); ?vision(VISION); .print("Sugar ",SUGAR); .print("Metabolism ",METABOLISM); .print("Vision ",VISION); .abolish(sugar(_1548)); .abolish(metabolism(_1549)); .abolish(vision(_1550)); .my_name(MYNAME); .concat("src/agt/list/",MYNAME,".asl",NAME); .save_agent(NAME,[start,say(hello)]); .print("Saved my information on file. Sending message to remove agent from simulation"); .send(killer_agent,tell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)); .send(killer_agent,untell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)).
@p__270[source(self),url("file:src/agt/default_agent.asl")] +free : (gsize(_1551,W,H) & (jia.random(RX,(W-1)) & jia.random(RY,(H-1)))) <- .print("I am going to go near (",RX,",",RY,")"); !go_near(RX,RY).
...
@p__273[source(self),url("file:src/agt/default_agent.asl")] +!go_near(X,Y) : free <- -near(_1552,_1553); -last_dir(_1554); !near(X,Y).
...
@p__279[source(self),url("file:src/agt/default_agent.asl")] +!next_step(X,Y) : not (pos(_1555,_1556)) <- !next_step(X,Y).
...
@pih1[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : .desire(near(_1557,_1558)) <- .print("Dropping near(_,_) desires and intentions to handle ",Gold); .drop_desire(near(_1559,_1560)); !init_handle(Gold).
@pih2[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : pos(X,Y) <- .print("Going for ",Gold); !!handle(Gold).
@pfunction[atomic,source(self),url("file:src/agt/default_agent.asl")] +!handle(gold(X,Y)) : not (free) <- .print("Handling ",gold(X,Y)," now."); !pos(X,Y); !ensure(pick,gold(X,Y)); ?depot(_1561,DX,DY); !pos(DX,DY); !ensure(drop,0); .print("Finish handling ",gold(X,Y)); ?score(S); -+score((S+1)); .send(leader,tell,dropped); +main.
...
@p__286[source(self),url("file:src/agt/default_agent.asl")] +!ensure(pick,_1562) : (pos(X,Y) & gold(X,Y)) <- pick; ?carrying_gold; -gold(X,Y).
@p__287[source(self),url("file:src/agt/default_agent.asl")] +!ensure(drop,_1563) : (carrying_gold & (pos(X,Y) & depot(_1564,X,Y))) <- drop.
...
@p__290[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : not (gold(_1565,_1566)) <- -+free.
@p__291[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : gold(_1567,_1568) <- .findall(gold(X,Y),gold(X,Y),LG); !calc_gold_distance(LG,LD); .length(LD,LLD); (LLD > 0); .print("Gold distances: ",LD,LLD); .min(LD,d(_1569,NewG)); .print("Next gold is ",NewG); !!handle(NewG).
...
@p__295[source(self),url("file:src/agt/default_agent.asl")] +!calc_gold_distance([_1570|R],RD) <- !calc_gold_distance(R,RD).
@p__296[source(self),url("file:src/agt/default_agent.asl")] +end_of_simulation(S,_1571) <- .drop_all_desires; .abolish(gold(_1572,_1573)); .abolish(picked(_1574)); -+free; .print("-- END ",S," --").
