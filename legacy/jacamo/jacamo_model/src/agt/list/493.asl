// beliefs and rules
kqml::bel_no_source_self(NS::Content,Ans) :- (NS::Content[|LA] & (kqml::clear_source_self(LA,NLA) & ((Content =.. [F,T,_612]) & (Ans =.. [NS,F,T,NLA])))).
...
focused(WksName,ArtName[artifact_type(Type)],ArtId) :- focusing(ArtId,ArtName,Type,_570,WksName,_571).
joinedWsp(cobj_2,mining,"/main/mining")[artifact_id(cobj_1),artifact_name(session_493),percept_type(obs_prop),source(percept),workspace("/main",cobj_0)].
joinedWsp(cobj_0,main,"/main")[artifact_id(cobj_1),artifact_name(session_493),percept_type(obs_prop),source(percept),workspace("/main",cobj_0)].
joined(WksName,WksId) :- joinedWsp(WksId,WksName,_572).
last_dir(down).
focusing(cobj_3,m2view,"mining.MiningPlanet",cobj_2,mining,"/main/mining")[artifact_id(cobj_4),artifact_name(body_493),percept_type(obs_prop),source(percept),workspace("/main/mining",cobj_2)].
...
@p__137[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art([],_567).
...
@p__139[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art(art_env(W,"",_568),Try) <- .concat("/main/",W,FullW); joinWorkspace(FullW,_569); .print("joinned workspace ",FullW).
...
@pfunction2[atomic,source(self),url("file:src/agt/default_agent.asl")] +main <- .print("Hello there. I'm a regular agent"); ?agent_id(AGENTID); ?path(PATH); .random(R); +my_testing(R); ?my_testing(R); .print("R: ",R); .abolish(agent_id(_573)); .abolish(path(_574)); ?sugar(SUGAR); ?metabolism(METABOLISM); ?vision(VISION); .print("Sugar ",SUGAR); .print("Metabolism ",METABOLISM); .print("Vision ",VISION); .abolish(sugar(_575)); .abolish(metabolism(_576)); .abolish(vision(_577)); .my_name(MYNAME); .concat("src/agt/list/",MYNAME,".asl",NAME); .save_agent(NAME,[start,say(hello)]); .print("Saved my information on file. Sending message to remove agent from simulation"); .send(killer_agent,tell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)); .send(killer_agent,untell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)).
@p__142[source(self),url("file:src/agt/default_agent.asl")] +free : (gsize(_578,W,H) & (jia.random(RX,(W-1)) & jia.random(RY,(H-1)))) <- .print("I am going to go near (",RX,",",RY,")"); !go_near(RX,RY).
...
@p__145[source(self),url("file:src/agt/default_agent.asl")] +!go_near(X,Y) : free <- -near(_579,_580); -last_dir(_581); !near(X,Y).
...
@p__151[source(self),url("file:src/agt/default_agent.asl")] +!next_step(X,Y) : not (pos(_582,_583)) <- !next_step(X,Y).
...
@pih1[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : .desire(near(_584,_585)) <- .print("Dropping near(_,_) desires and intentions to handle ",Gold); .drop_desire(near(_586,_587)); !init_handle(Gold).
@pih2[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : pos(X,Y) <- .print("Going for ",Gold); !!handle(Gold).
@pfunction[atomic,source(self),url("file:src/agt/default_agent.asl")] +!handle(gold(X,Y)) : not (free) <- .print("Handling ",gold(X,Y)," now."); !pos(X,Y); !ensure(pick,gold(X,Y)); ?depot(_588,DX,DY); !pos(DX,DY); !ensure(drop,0); .print("Finish handling ",gold(X,Y)); ?score(S); -+score((S+1)); .send(leader,tell,dropped); +main.
...
@p__158[source(self),url("file:src/agt/default_agent.asl")] +!ensure(pick,_589) : (pos(X,Y) & gold(X,Y)) <- pick; ?carrying_gold; -gold(X,Y).
@p__159[source(self),url("file:src/agt/default_agent.asl")] +!ensure(drop,_590) : (carrying_gold & (pos(X,Y) & depot(_591,X,Y))) <- drop.
...
@p__162[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : not (gold(_592,_593)) <- -+free.
@p__163[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : gold(_594,_595) <- .findall(gold(X,Y),gold(X,Y),LG); !calc_gold_distance(LG,LD); .length(LD,LLD); (LLD > 0); .print("Gold distances: ",LD,LLD); .min(LD,d(_596,NewG)); .print("Next gold is ",NewG); !!handle(NewG).
...
@p__167[source(self),url("file:src/agt/default_agent.asl")] +!calc_gold_distance([_597|R],RD) <- !calc_gold_distance(R,RD).
@p__168[source(self),url("file:src/agt/default_agent.asl")] +end_of_simulation(S,_598) <- .drop_all_desires; .abolish(gold(_599,_600)); .abolish(picked(_601)); -+free; .print("-- END ",S," --").
