// beliefs and rules
kqml::bel_no_source_self(NS::Content,Ans) :- (NS::Content[|LA] & (kqml::clear_source_self(LA,NLA) & ((Content =.. [F,T,_1764]) & (Ans =.. [NS,F,T,NLA])))).
...
focused(WksName,ArtName[artifact_type(Type)],ArtId) :- focusing(ArtId,ArtName,Type,_1722,WksName,_1723).
joinedWsp(cobj_2,mining,"/main/mining")[artifact_id(cobj_1),artifact_name(session_416),percept_type(obs_prop),source(percept),workspace("/main",cobj_0)].
joinedWsp(cobj_0,main,"/main")[artifact_id(cobj_1),artifact_name(session_416),percept_type(obs_prop),source(percept),workspace("/main",cobj_0)].
joined(WksName,WksId) :- joinedWsp(WksId,WksName,_1724).
last_dir(down).
focusing(cobj_3,m2view,"mining.MiningPlanet",cobj_2,mining,"/main/mining")[artifact_id(cobj_4),artifact_name(body_416),percept_type(obs_prop),source(percept),workspace("/main/mining",cobj_2)].
...
@p__265[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art([],_1719).
...
@p__267[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art(art_env(W,"",_1720),Try) <- .concat("/main/",W,FullW); joinWorkspace(FullW,_1721); .print("joinned workspace ",FullW).
...
@pfunction2[atomic,source(self),url("file:src/agt/default_agent.asl")] +main <- .print("Hello there. I'm a regular agent"); ?agent_id(AGENTID); ?path(PATH); .random(R); +my_testing(R); ?my_testing(R); .print("R: ",R); .abolish(agent_id(_1725)); .abolish(path(_1726)); ?sugar(SUGAR); ?metabolism(METABOLISM); ?vision(VISION); .print("Sugar ",SUGAR); .print("Metabolism ",METABOLISM); .print("Vision ",VISION); .abolish(sugar(_1727)); .abolish(metabolism(_1728)); .abolish(vision(_1729)); .my_name(MYNAME); .concat("src/agt/list/",MYNAME,".asl",NAME); .save_agent(NAME,[start,say(hello)]); .print("Saved my information on file. Sending message to remove agent from simulation"); .send(killer_agent,tell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)); .send(killer_agent,untell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)).
@p__270[source(self),url("file:src/agt/default_agent.asl")] +free : (gsize(_1730,W,H) & (jia.random(RX,(W-1)) & jia.random(RY,(H-1)))) <- .print("I am going to go near (",RX,",",RY,")"); !go_near(RX,RY).
...
@p__273[source(self),url("file:src/agt/default_agent.asl")] +!go_near(X,Y) : free <- -near(_1731,_1732); -last_dir(_1733); !near(X,Y).
...
@p__279[source(self),url("file:src/agt/default_agent.asl")] +!next_step(X,Y) : not (pos(_1734,_1735)) <- !next_step(X,Y).
...
@pih1[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : .desire(near(_1736,_1737)) <- .print("Dropping near(_,_) desires and intentions to handle ",Gold); .drop_desire(near(_1738,_1739)); !init_handle(Gold).
@pih2[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : pos(X,Y) <- .print("Going for ",Gold); !!handle(Gold).
@pfunction[atomic,source(self),url("file:src/agt/default_agent.asl")] +!handle(gold(X,Y)) : not (free) <- .print("Handling ",gold(X,Y)," now."); !pos(X,Y); !ensure(pick,gold(X,Y)); ?depot(_1740,DX,DY); !pos(DX,DY); !ensure(drop,0); .print("Finish handling ",gold(X,Y)); ?score(S); -+score((S+1)); .send(leader,tell,dropped); +main.
...
@p__286[source(self),url("file:src/agt/default_agent.asl")] +!ensure(pick,_1741) : (pos(X,Y) & gold(X,Y)) <- pick; ?carrying_gold; -gold(X,Y).
@p__287[source(self),url("file:src/agt/default_agent.asl")] +!ensure(drop,_1742) : (carrying_gold & (pos(X,Y) & depot(_1743,X,Y))) <- drop.
...
@p__290[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : not (gold(_1744,_1745)) <- -+free.
@p__291[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : gold(_1746,_1747) <- .findall(gold(X,Y),gold(X,Y),LG); !calc_gold_distance(LG,LD); .length(LD,LLD); (LLD > 0); .print("Gold distances: ",LD,LLD); .min(LD,d(_1748,NewG)); .print("Next gold is ",NewG); !!handle(NewG).
...
@p__295[source(self),url("file:src/agt/default_agent.asl")] +!calc_gold_distance([_1749|R],RD) <- !calc_gold_distance(R,RD).
@p__296[source(self),url("file:src/agt/default_agent.asl")] +end_of_simulation(S,_1750) <- .drop_all_desires; .abolish(gold(_1751,_1752)); .abolish(picked(_1753)); -+free; .print("-- END ",S," --").
