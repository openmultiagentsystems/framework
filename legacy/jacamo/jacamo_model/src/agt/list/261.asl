// beliefs and rules
kqml::bel_no_source_self(NS::Content,Ans) :- (NS::Content[|LA] & (kqml::clear_source_self(LA,NLA) & ((Content =.. [F,T,_314]) & (Ans =.. [NS,F,T,NLA])))).
...
my_testing(0.30882639362411146).
...
focused(WksName,ArtName[artifact_type(Type)],ArtId) :- focusing(ArtId,ArtName,Type,_272,WksName,_273).
joinedWsp(cobj_2,mining,"/main/mining")[artifact_id(cobj_1),artifact_name(session_261),percept_type(obs_prop),source(percept),workspace("/main",cobj_0)].
joinedWsp(cobj_0,main,"/main")[artifact_id(cobj_1),artifact_name(session_261),percept_type(obs_prop),source(percept),workspace("/main",cobj_0)].
joined(WksName,WksId) :- joinedWsp(WksId,WksName,_274).
last_dir(down).
focusing(cobj_3,m2view,"mining.MiningPlanet",cobj_2,mining,"/main/mining")[artifact_id(cobj_4),artifact_name(body_261),percept_type(obs_prop),source(percept),workspace("/main/mining",cobj_2)].
...
@p__41[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art([],_269).
...
@p__43[source(self),url("jar:file:/home/gradle/.gradle/caches/modules-2/files-2.1/org.jacamo/jacamo/1.0/bda076187adb93bc05a91ab0cdcb2fb44b039ed6/jacamo-1.0.jar!/templates/common-cartago.asl")] +!jcm::focus_env_art(art_env(W,"",_270),Try) <- .concat("/main/",W,FullW); joinWorkspace(FullW,_271); .print("joinned workspace ",FullW).
...
@pfunction2[atomic,source(self),url("file:src/agt/default_agent.asl")] +main <- .print("Hello there. I'm a regular agent"); ?agent_id(AGENTID); ?path(PATH); .random(R); +my_testing(R); ?my_testing(R); .print("R: ",R); .abolish(agent_id(_275)); .abolish(path(_276)); ?sugar(SUGAR); ?metabolism(METABOLISM); ?vision(VISION); .print("Sugar ",SUGAR); .print("Metabolism ",METABOLISM); .print("Vision ",VISION); .abolish(sugar(_277)); .abolish(metabolism(_278)); .abolish(vision(_279)); .my_name(MYNAME); .concat("src/agt/list/",MYNAME,".asl",NAME); .save_agent(NAME,[start,say(hello)]); .print("Saved my information on file. Sending message to remove agent from simulation"); .send(killer_agent,tell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)); .send(killer_agent,untell,kill(AGENTID,PATH,MYNAME,SUGAR,METABOLISM,VISION)).
@p__46[source(self),url("file:src/agt/default_agent.asl")] +free : (gsize(_280,W,H) & (jia.random(RX,(W-1)) & jia.random(RY,(H-1)))) <- .print("I am going to go near (",RX,",",RY,")"); !go_near(RX,RY).
...
@p__49[source(self),url("file:src/agt/default_agent.asl")] +!go_near(X,Y) : free <- -near(_281,_282); -last_dir(_283); !near(X,Y).
...
@p__55[source(self),url("file:src/agt/default_agent.asl")] +!next_step(X,Y) : not (pos(_284,_285)) <- !next_step(X,Y).
...
@pih1[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : .desire(near(_286,_287)) <- .print("Dropping near(_,_) desires and intentions to handle ",Gold); .drop_desire(near(_288,_289)); !init_handle(Gold).
@pih2[atomic,source(self),url("file:src/agt/default_agent.asl")] +!init_handle(Gold) : pos(X,Y) <- .print("Going for ",Gold); !!handle(Gold).
@pfunction[atomic,source(self),url("file:src/agt/default_agent.asl")] +!handle(gold(X,Y)) : not (free) <- .print("Handling ",gold(X,Y)," now."); !pos(X,Y); !ensure(pick,gold(X,Y)); ?depot(_290,DX,DY); !pos(DX,DY); !ensure(drop,0); .print("Finish handling ",gold(X,Y)); ?score(S); -+score((S+1)); .send(leader,tell,dropped); +main.
...
@p__62[source(self),url("file:src/agt/default_agent.asl")] +!ensure(pick,_291) : (pos(X,Y) & gold(X,Y)) <- pick; ?carrying_gold; -gold(X,Y).
@p__63[source(self),url("file:src/agt/default_agent.asl")] +!ensure(drop,_292) : (carrying_gold & (pos(X,Y) & depot(_293,X,Y))) <- drop.
...
@p__66[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : not (gold(_294,_295)) <- -+free.
@p__67[source(self),url("file:src/agt/default_agent.asl")] +!choose_gold : gold(_296,_297) <- .findall(gold(X,Y),gold(X,Y),LG); !calc_gold_distance(LG,LD); .length(LD,LLD); (LLD > 0); .print("Gold distances: ",LD,LLD); .min(LD,d(_298,NewG)); .print("Next gold is ",NewG); !!handle(NewG).
...
@p__71[source(self),url("file:src/agt/default_agent.asl")] +!calc_gold_distance([_299|R],RD) <- !calc_gold_distance(R,RD).
@p__72[source(self),url("file:src/agt/default_agent.asl")] +end_of_simulation(S,_300) <- .drop_all_desires; .abolish(gold(_301,_302)); .abolish(picked(_303)); -+free; .print("-- END ",S," --").
