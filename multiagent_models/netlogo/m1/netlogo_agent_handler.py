...
def testing_exception(error):
    """ Function create for testing only,
    testing the creation of a custom exception """
    raise Exception(error)


def request_to_register(model, min, max):
    """ Function that requests to register to create a
    certain amount of agents. Asked by NetLogo.
    Calls the register_agents_on_platform endpoint on API """
    ...
    while should_return == False:
        try:
            response = requests.post('http://'+host+':'+port+'/api/v1/resources/register_agents_on_platform', json={
                                     "model": model, "min": min, "max": max}, timeout=120)
    ...
    while should_return == False:
        try:
            response = requests.post(
                'http://'+host+':'+port+'/api/v1/resources/model_to_alive',
                json={
                    "agent_id": agent_id, "model": model}, timeout=120
            )
...
def send_agent_to_router(agent_id, data, path):
    ...
    while should_return == False:
        try:
            response = requests.post('http://'+host+':'+port+'/api/v1/resources/model_to_router', json={
                                     "agent_id": agent_id, "data": data, "path": path}, timeout=120)
            response.raise_for_status()
    ...
    while should_return == False:
        try:
            response = requests.get(
                'http://'+host+':'+port+'/api/v1/resources/check_new_agents', params={"model": modelo}, timeout=120)
            response.raise_for_status()
